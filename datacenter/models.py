...
class Target(models.Model):
    ...
    magnification = models.DecimalField("倍率", max_digits=18, decimal_places=5)
    digit = models.IntegerField("保留位数", blank=True, null=True)
    cumulative = models.CharField("是否累计", max_length=20)
    upperlimit = models.DecimalField("上限", max_digits=20, decimal_places=5)
    lowerlimit = models.DecimalField("下限", max_digits=20, decimal_places=5)
    formula = models.CharField("公式", max_length=2000)
    cycle = models.ForeignKey(Cycle,null=True)
    source = models.ForeignKey(Source,null=True)
    sourcetable = models.CharField("数据源表", max_length=2000)
    sourcefields = models.CharField("数据源字段", max_length=2000)
    sourceconditions = models.CharField("数据源条件", max_length=2000)
    sourcesis = models.CharField("数据源sis点", max_length=2000)
    storage = models.ForeignKey(Storage,null=True)
    storagefields = models.CharField("存储字段", max_length=2000)
    storagetag = models.CharField("存储标识", max_length=200)
    sort = models.IntegerField("排序", blank=True, null=True)
    state = models.CharField("状态", blank=True, null=True, max_length=20)
    remark = models.CharField("说明", blank=True, null=True, max_length=2000)
