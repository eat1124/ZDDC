...
$(document).ready(function () {
    ...
    $('#collect').click(function () {
        ...
        // 筛选框中的数据
        var tmp = "<select style='width:100px'><option value='d'>当前值</option><option value='m'>月累计</option><option value='s'>季累计</option><option value='h'>半年累计</option><option value='y'>年累计</option></select>";

        if ($('#cumulative option:selected').val()=='否') {
            tmp = "<select style='width:100px'><option value='d'>当前值</option>";
        } else if ($('#cumulative option:selected').val() =='是' && $('#cycletype option:selected').val()=='10'){
            tmp = tmp
        } else if ($('#cumulative option:selected').val() =='是' && $('#cycletype option:selected').val()=='11'){
            tmp = "<select style='width:100px'><option value='d'>当前值</option><option value='s'>季累计</option><option value='h'>半年累计</option><option value='y'>年累计</option></select>";
        }else if ($('#cumulative option:selected').val() =='是' && $('#cycletype option:selected').val()=='12'){
            tmp = "<select style='width:100px'><option value='d'>当前值</option><option value='h'>半年累计</option><option value='y'>年累计</option></select>";
        }else if ($('#cumulative option:selected').val() =='是' && $('#cycletype option:selected').val()=='13'){
            tmp = "<select style='width:100px'><option value='d'>当前值</option><option value='y'>年累计</option></select>";
        } else {
            tmp = "<select style='width:100px'><option value='d'>当前值</option>";
        }

        // 模态框表格数据
        $("#sample_3").dataTable().fnDestroy(); // 还原初始化了的dataTable
        $('#sample_3').dataTable({
            "bAutoWidth": true,
            "bSort": false,
            "bProcessing": true,
