...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(This.$id+"_node_"+( "0000000000" + This.$max ).substr( -10 ),{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
/*		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
	...
	initGroup:function(width,height){
      ...
      e.data.inthis.addArea(e.data.inthis.$id+"_area_"+( "0000000000" + e.data.inthis.$max ).substr( -10 ),{name:"area_"+e.data.inthis.$max,left:X,top:Y,color:color[e.data.inthis.$max%4],width:200,height:100});
      e.data.inthis.$max++;
      return false;
		}
	  });
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				This.addLine(This.$id+"_line_"+( "0000000000" + This.$max ).substr( -10 ),{from:lineStart.id,to:this.id,name:""});
				This.$max++;
			}
			else{
				if(lineStart){
